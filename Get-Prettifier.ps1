...
function Get-Prettifier
{
    ...
    # If provided, will get Prettifier that extend a given command
    [Parameter(ValueFromPipelineByPropertyName)]
    ...
    begin {
        ...
        filter ConvertToExtension {
            ...
            if ($PrettifierTypeName) {
                $extCmd.pstypenames.add($PrettifierTypeName)
            } else {
        ...
        $extensionFullRegex =
            [Regex]::New($(
                if ($PrettifierModule) {
                    "\.(?>$(@(@($PrettifierModule) + $PrettifierModuleAlias) -join '|'))\." + "(?>$($PrettifierPattern -join '|'))"
                } else {
                    "(?>$($PrettifierPattern -join '|'))"
                }
        ...
        $myInv = $MyInvocation
        $myModuleName = if ($PrettifierModule) { $PrettifierModule } else { $MyInvocation.MyCommand.Module.Name }
        ...
        if ($Force) {
            $script:Prettifiers  = $null
            $script:AllCommands = @()
        }
        if (-not $script:Prettifiers)
        {
            $script:Prettifiers =
                @(
                #region Find Prettifier in Loaded Modules
                ...
                #endregion Find Prettifier in Loaded Modules
                )
        }
    ...
    process {
        ...
        if ($PrettifierPath) {
            Get-ChildItem -Recurse -Path $PrettifierPath |
                Where-Object { $_.Name -Match $extensionFullRegex } |
                ConvertToExtension |
